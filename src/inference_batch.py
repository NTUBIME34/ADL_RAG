...
BATCH_Q = 32 # you can adjust this value to fit hardware constraints
...
# For decoder-only models, prefer left padding and left truncation so the
# most recent tokens (question + contexts + "final_answer:") are preserved
# when inputs are too long.
tokenizer.padding_side = "left"
tokenizer.truncation_side = "left"
model = AutoModelForCausalLM.from_pretrained(model_id, dtype="auto", device_map="auto")
# Derive a safe maximum input length to avoid losing context silently.
try:
    _model_ctx = int(getattr(model.config, "max_position_embeddings", 4096))
except Exception:
    _model_ctx = 4096
MAX_INPUT_LEN = max(256, _model_ctx - GEN_MAXLEN - 64)
        ...
        with torch.no_grad():
            outs = model.generate(
                **inputs,
                ...
                do_sample=False,           # deterministic, more faithful to prompt
                temperature=0.0,
                top_p=1.0,
